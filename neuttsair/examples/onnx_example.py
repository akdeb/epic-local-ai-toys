...
def main(input_text, ref_codes_path, ref_text, backbone, output_path="output.wav"):
    ...
    wav = tts.infer("my name is dave", ref_codes, ref_text)
    end_time = time.time()
    print(f"Time taken: {end_time - start_time} seconds")

    print(f"Saving output to {output_path}")
    sf.write("1"+output_path, wav, 24000)
    start_time = time.time()
    wav = tts.infer("and um", ref_codes, ref_text)
    end_time = time.time()
    print(f"Time taken: {end_time - start_time} seconds")

    print(f"Saving output to {output_path}")
    sf.write("2"+output_path, wav, 24000)

    start_time = time.time()
    wav = tts.infer("i'm from London in england.", ref_codes, ref_text)
    end_time = time.time()
    print(f"Time taken: {end_time - start_time} seconds")

    print(f"Saving output to {output_path}")
    sf.write("3"+output_path, wav, 24000)
