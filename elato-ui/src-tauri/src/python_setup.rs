...
fn resolve_pyproject_path(app: &AppHandle) -> Result<PathBuf, String> {
    let repo_pyproject_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..")
        .join("resources")
        .join("python")
    ...
    if repo_pyproject_path.exists() {
        return Ok(repo_pyproject_path);
    }

    let resource_dir = app.path().resource_dir().map_err(|e| e.to_string())?;
    Ok(resource_dir.join("python").join("pyproject.toml"))
}
